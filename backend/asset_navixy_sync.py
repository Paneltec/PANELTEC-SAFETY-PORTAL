...
async def _sync_org(org_id: str) -> dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            ...
            # Phase 4.9 — Pass 0.5: canonical Navixy counter endpoint.
            # `/v2/tracker/get_counters` is the endpoint that actually returns
            # lifetime engine_hours + odometer for every tracker. The legacy
            # `counter/read` / `counter/list` paths return HTTP 400 on the
            # current Navixy plan. Runs for EVERY synced asset, warm or cold,
            # so values stay fresh. Idempotent — only `_apply_counters` writes,
            # and that helper refuses to overwrite a higher value with a
            # transient lower one.
                    ...
                    try:
                        ...
                        if r.status_code < 400:
                            ...
                            if slot["hours"] is not None or slot["odo_km"] is not None:
                                ...
                                log.info("navixy.sync device_id=%s hours=%s km=%s source=counters_v2",
                                         tid, slot["hours"], slot["odo_km"])
                    except (httpx.HTTPError, ValueError) as e:
                        errors += 1
                        log.info("get_counters tid=%s err=%s", tid, e)
