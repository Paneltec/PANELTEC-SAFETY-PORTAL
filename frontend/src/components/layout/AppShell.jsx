...
import { WORKSPACES } from '../../mocks/dashboard';
import {
...
const TopBar = ({ onToggleMobile, onToggleCollapse, collapsed }) => {
  const navigate = useNavigate();
  const user = getUser() || { name: 'Demo User', email: 'demo@paneltec.com', initials: 'DU' };
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 flex items-center gap-3 px-4 lg:px-6 sticky top-0 z-30">
      ...
      <button
        className="hidden md:inline-flex p-2 rounded-md hover:bg-slate-100 text-slate-500"
      ...
      <div className="hidden md:flex flex-1 max-w-md ml-2 relative">
        ...
        <input
          type="text"
          placeholder="Search records, contractors, SWMS…"
      ...
      >
        <Bell size={18} />
        <span className="absolute top-1 right-1 w-4 h-4 rounded-full bg-brand-red text-white text-[10px] font-semibold flex items-center justify-center">
          3
        </span>
      </button>
...
const SidebarShell = ({ collapsed, onItemClick }) => (
  <aside
    className={`hidden md:flex flex-col bg-white border-r border-slate-200 transition-[width] duration-200 ${
...
export default function AppShell() {
  ...
  // gate — redirect via <Navigate /> instead of imperative navigate during render
  if (!getUser()) {
