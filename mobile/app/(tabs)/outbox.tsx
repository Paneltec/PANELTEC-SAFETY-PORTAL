...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string }> = {
  queued: { bg: '#FEF3C7', text: '#B45309', border: '#FDE68A' },
  sent: { bg: '#D1FAE5', text: '#047857', border: '#A7F3D0' },
...
export default function OutboxScreen() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data: d } = await api.get('/email/outbox', { params });
      setData(d);
    } catch (e) {
  ...
  const retry = async (id: string) => {
    try { await api.post(`/email/outbox/${id}/retry`); Alert.alert('Retried'); load(); }
    catch (e) { Alert.alert('Error', apiError(e)); }
  };
  const cancel = async (id: string) => {
    try { await api.post(`/email/outbox/${id}/cancel`); Alert.alert('Cancelled'); load(); }
    catch (e) { Alert.alert('Error', apiError(e)); }
  };
      ...
      >
        ...
        {!data.m365_connected && (
          <View testID="m365-banner" style={s.warnBanner}>
            <Ionicons name="alert-triangle" size={16} color="#B45309" />
            <Text style={s.warnText}>Microsoft 365 not connected. Queued emails won't send until M365 is configured in Settings → Integrations.</Text>
          </View>
        )}
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={s.filterRow}>
          {statuses.map((st, i) => (
            <TouchableOpacity
              key={st}
              testID={`outbox-filter-${st || 'all'}`}
              style={[s.filterChip, statusF === st && s.filterActive]}
              onPress={() => setStatusF(st)}
            >
        ...
        {loading ? <ActivityIndicator style={{ marginTop: 40 }} color={Colors.blue} /> :
          data.items.length === 0 ? (
            <View style={s.emptyBox}>
              ...
              <Text style={s.emptyText}>No outbox messages</Text>
            </View>
          ) :
          data.items.map(m => {
              ...
              <TouchableOpacity key={m.id} testID={`outbox-row-${m.id}`} style={s.card} onPress={() => setActive(m)} activeOpacity={0.7}>
                <View style={s.cardTop}>
                  <View style={[s.statusPill, { backgroundColor: sc.bg, borderColor: sc.border }]}>
                    <Text style={[s.statusText, { color: sc.text }]}>{m.status}</Text>
                  </View>
                ...
                {m.related_record_type && <Text style={s.cardType}>{m.related_record_type}</Text>}
                <View style={s.cardActions}>
                  ...
                  {m.status === 'queued' && (
                    <TouchableOpacity testID={`outbox-cancel-${m.id}`} style={[s.actionBtn, s.cancelActionBtn]} onPress={() => cancel(m.id)}>
        ...
        <SafeAreaView style={s.drawerSafe}>
          <View testID="outbox-drawer" style={s.drawer}>
            <View style={s.drawerHeader}>
              ...
              <TouchableOpacity onPress={() => setActive(null)} style={s.closeBtn}>
                <Ionicons name="close" size={22} color={Colors.textTertiary} />
              </TouchableOpacity>
            </View>
            {active && (
              <ScrollView style={s.drawerBody}>
                {(() => { const sc = STATUS_COLORS[active.status] || STATUS_COLORS.queued; return (
                  <View style={[s.statusPill, { backgroundColor: sc.bg, borderColor: sc.border, alignSelf: 'flex-start', marginBottom: 12 }]}>
                    <Text style={[s.statusText, { color: sc.text }]}>{active.status}</Text>
                  </View>
                ); })()}
                <Text style={s.drawerSubject}>{active.subject}</Text>
                <View style={s.drawerField}>
                  <Ionicons name="mail" size={12} color={Colors.textTertiary} />
                  <Text style={s.drawerFieldText}>{(active.to || []).join(', ')}</Text>
                </View>
                {active.cc?.length > 0 && (
                  <View style={s.drawerField}>
                    <Text style={s.drawerFieldLabel}>CC:</Text>
                    <Text style={s.drawerFieldText}>{active.cc.join(', ')}</Text>
                  </View>
                )}
                {active.related_record_type && (
                  <View style={s.drawerField}>
                    <Text style={s.drawerFieldLabel}>Related:</Text>
                    <Text style={s.drawerFieldText}>{active.related_record_type}</Text>
                  </View>
                )}
                <View style={s.drawerField}>
                  <Text style={s.drawerFieldLabel}>Created:</Text>
                  <Text style={s.drawerFieldText}>{new Date(active.created_at).toLocaleString()}</Text>
                </View>
                {active.error && (
                  <View style={s.errorBox}>
                    <Text style={s.errorText}>Error: {active.error}</Text>
                  </View>
                )}
              </ScrollView>
            )}
...
const s = StyleSheet.create({
  ...
  warnBanner: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, marginTop: 12, padding: 12, borderRadius: 12, backgroundColor: Colors.amberSoft, borderWidth: 1, borderColor: '#FDE68A' },
  warnText: { fontSize: 13, color: '#B45309', flex: 1, lineHeight: 18 },
  filterRow: { marginTop: 14, marginBottom: 14, flexGrow: 0 },
  filterChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 10, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.white, marginRight: 8 },
  ...
  cardType: { fontSize: 11, color: Colors.textTertiary, marginTop: 4 },
  ...
  actionBtnText: { fontSize: 12, fontWeight: '500', color: Colors.blue },
  cancelActionBtn: { borderColor: Colors.redSoft },
  // Drawer
  drawerSafe: { flex: 1, backgroundColor: Colors.bg },
  drawer: { flex: 1 },
  drawerHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: Colors.border, backgroundColor: Colors.white },
  ...
  drawerFieldLabel: { fontSize: 12, fontWeight: '600', color: Colors.textTertiary },
  drawerFieldText: { fontSize: 13, color: Colors.textSecondary, flex: 1 },
  errorBox: { marginTop: 12, padding: 10, borderRadius: 10, backgroundColor: Colors.redSoft, borderWidth: 1, borderColor: '#FECACA' },
